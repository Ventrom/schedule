...
schedule.memoizedRangeFn = function (fn) {
  ...
  return function (start) {
    if (!cache[start]) {
      ...
      cache[start] = [
        result[0] ? result[0].getTime() : 41024448e5,
        result[1] ? result[1].getTime() : 41024448e5,
      ];
    }
    return cache[start];
  };
...
schedule.dependencyGraph = function (taskArr) {
  ...
  function forwardPass(graph, depEnds, tasks, start) {
    ...
    for (var i = 0, len = tasks.length; i < len; i++) {
      ...
      if (
        !task.earlyFinish &&
        (isEmpty(dependsOn) || (dep && dep[0] === dependsOn.length))
      ) {
...
schedule.resourceManager = function (resourceDefinitions, startDate) {
  ...
  function getReservation(resources, start, min, max) {
    ...
    while (
      !(reservation = tryReservation(schedules, min, max)).success &&
      --maxTries
    ) {
  ...
  function initRanges(resources, start, ranges, delays) {
    for (var i = 0, len = resources.length; i < len; i++) {
      ...
      if (Array.isArray(resId)) {
        ...
        var res = rMap[resId],
          range = res.nextAvail[0] >= start ? res.nextAvail : res.next(start);
  ...
  function applyReservation(resources, start, end) {
    for (var i = 0, len = resources.length; i < len; i++) {
      ...
      if (start !== res.nextAvail[0]) {
        ...
        res.next = schedule.memoizedRangeFn(
          later.schedule(res.schedule).nextRange
        );
        end = res.nextAvail[0];
      }
